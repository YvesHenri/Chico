...
public:
	explicit QuadTree(float x, float y, float width, float height)
		: QuadTree(x, y, width, height, 64.f, 64.f, 4U, 0U) {}

	explicit QuadTree(float x, float y, float width, float height, float minWidth, float minHeight, unsigned capacity, unsigned level)
		: x(x), y(y), width(width), height(height), minWidth(minWidth), minHeight(minHeight), capacity(capacity), level(level) {
	...
	void query(float x, float y, float radius, std::vector<std::shared_ptr<QuadNode>>& result) {
		...
		if (Geometry::intersects(x, y, radius, this->x, this->y, this->width, this->height)) {
			for (auto& node : nodes) {
	...
	void query(float x, float y, float width, float height, std::vector<std::shared_ptr<QuadNode>>& result) {
		// Skip quad if it does not intersect with the search boundary
		if (Geometry::intersects(x, y, width, height, this->x, this->y, this->width, this->height)) {
			for (auto& node : nodes) {
...
private:
	...
	void split() {
		...
		if (nodes.size() > capacity && subWidth > minWidth && subHeight > minHeight && unsplit) {
			northwest = std::make_shared<QuadTree>(x, y, subWidth, subHeight, minWidth, minHeight, capacity, level + 1U);
			northeast = std::make_shared<QuadTree>(x + subWidth, y, subWidth, subHeight, minWidth, minHeight, capacity, level + 1U);
			southwest = std::make_shared<QuadTree>(x, y + subHeight, subWidth, subHeight, minWidth, minHeight, capacity, level + 1U);
			southeast = std::make_shared<QuadTree>(x + subWidth, y + subHeight, subWidth, subHeight, minWidth, minHeight, capacity, level + 1U);
